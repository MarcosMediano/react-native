...
class BadgesEdit extends react.Component{
    ...
    //gets badge info
    getBadge = () => {
        const {item} = this.props.route.params; //gets data
        this.setState({badge: item}); //sends the item to the badge
        this.props.navigation.setOptions({title: `Edit ${item.name}`}); //displays the name 
    };
    // saves and sends the edited data
    handleSubmit = async () =>{
